...
| SQL | Local SQLite lean schema connected | Keep SQLite as working local store; design adapter seam for future proprietary SQL | Provide proprietary SQL connection details later, not required for spike | Value snapshots, weight snapshots, position states, data-quality events, and audit summaries are stored through the SQL MCP boundary |
