...
These files break down the V1.2 implementation plan from
[`ACTION_PLAN.md`](../ACTION_PLAN.md). V1.1 task maps remain historical under
`../V1_1_Tasks/`.

Status: V1.2 skeleton complete as of 2026-06-15.

V1.2 implemented goal:

```text
V1.1 Portfolio Agent POC
...
V1.1 proved the Portfolio Agent POC:
...
V1.2 adds the first Investment Agent skeleton:

- real LangGraph `StateGraph` supervisor in `InvestmentAgent`
...
- deterministic tests for V1.2 contracts, graph routing, portfolio planning,
...
- V1.2 chat route through `investment`
- terminal V1.2 review script at `scripts/investment_agent_review.py`
...
- V1.2 synthesis is deterministic/template-style. It is not yet a rich LLM
...
- At V1.2 closeout, MCP modules had stdio JSON-RPC server wrappers, while agents
  still called in-process MCP modules rather than an official MCP client/host
  runtime. This was later superseded by V1.3.4, where Portfolio Agent and V1.2
...
Final deterministic gate for V1.2 closeout:
...
| Task | File | Purpose |
| --- | --- | --- |
| 1 | [TASK_1_DEFINE_V1_2_CONTRACTS.md](TASK_1_DEFINE_V1_2_CONTRACTS.md) | Complete. Create Pydantic contracts for Investment Agent state, query plans, Portfolio Agent tasks/plans, Sentiment Agent stub tasks, synthesis, and guardrails. |
...
| 3 | [TASK_3_PORTFOLIO_AGENT_BOUNDED_PLANNING.md](TASK_3_PORTFOLIO_AGENT_BOUNDED_PLANNING.md) | Complete. Convert the current deterministic Portfolio Agent into a bounded-planning path while preserving V1.1 behavior as the broad-review default. |
...
| 5 | [TASK_5_GUARDRAILS_AND_TRACE.md](TASK_5_GUARDRAILS_AND_TRACE.md) | Complete. Move deterministic guardrails and sanitized trace into the V1.2 Investment Agent path. |
| 6 | [TASK_6_DOCUMENTATION_AND_TESTS.md](TASK_6_DOCUMENTATION_AND_TESTS.md) | Complete. Add the test strategy and documentation updates needed to close V1.2. |
...
- T1-B: Decide V1.2 model module locations and naming.
...
Implementation tasks are complete for the V1.2 skeleton. Do not treat V1.2 as a
