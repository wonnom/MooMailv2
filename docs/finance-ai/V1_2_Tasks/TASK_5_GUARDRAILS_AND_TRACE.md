...
Move guardrail review and trace into the V1.2 Investment Agent path.

V1.2 should make graph progress, subagent calls, selected/skipped tools, failures,
...
The V1.2 Investment Agent emits:
...
Deterministic V1.2 guardrail check names:
...
A. Task 1 GuardrailReview and trace/status contracts
   ├── B. Define V1.2 guardrail checks
...
| B | Define V1.2 guardrail check names and severities. | A | Done |
...
| Task | Description | Depends on | Test |
| --- | --- | --- | --- |
| H | Define V1.2 trace entries for graph node start/end, subagent call, MCP tool call, skipped tool, and error. | A | Done |
...
| J1 | Extend existing frontend technical trace to show V1.2 graph trace without breaking V1.1 portfolio trace. | H, J | Done |
...
| H5 | Ensure audit/run summaries store output summary and tool/source refs only. | H4 | Deferred; V1.2 audit persistence is not active yet |
...
- Updates to `tests/test_chat_app.py` for V1.2 stream trace if frontend is wired.
...
- Stream error handling from V1.1 remains intact.
