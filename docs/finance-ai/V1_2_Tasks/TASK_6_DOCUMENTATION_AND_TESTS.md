...
Close V1.2 with deterministic tests and documentation that describe the actual
implemented architecture.

This task should be done throughout V1.2, not only at the end.
...
3. Docs describe V1.1 as complete and V1.2 skeleton as complete, including the
   remaining mocks/stubs/deferred pieces.

## Implementation Notes

Task 6 closes the V1.2 skeleton, not the whole long-term Finance AI.
...
- chat/static frontend V1.2 integration checks in `tests/test_chat_app.py`

Reality captured in docs:

- V1.2 uses a real LangGraph supervisor.
...
- V1.2 final synthesis is deterministic/template-style, not a full LLM
...
| --- | --- | --- | --- |
| A | Add `tests/test_agent_schemas.py`. | Task 1 | Done |
| B | Add `tests/test_investment_agent.py` with fake Portfolio Agent and fake Sentiment Agent. | Task 2 | Done |
| C | Add `tests/test_portfolio_planner.py`. | Task 3 | Done |
| D | Add `tests/test_sentiment_agent_stub.py`. | Task 4 | Done |
| E | Add `tests/test_investment_guardrails.py` and `tests/test_agent_trace.py`. | Task 5 | Done |
| F | Add V1.2 fixtures under `tests/fixtures/agent/` or local factory functions. | A through E | Done |
...
| H | Run `.venv/bin/python -m pytest tests --ignore=tests/live -q` and update doc counts only when final V1.2 closeout is reached. | A through G | Done: 156 passed, 1 warning |

### EC2: Live OpenD connector tests remain opt-in

| Task | Description | Depends on | Test |
| --- | --- | --- | --- |
| H1 | Confirm no new V1.2 deterministic test requires `MOOMAIL_RUN_LIVE_CONNECTOR_TESTS=1`. | A through E | Done |
| H2 | Keep live OpenD tests under `tests/live/`. | None | Done |
| H3 | If V1.2 adds a live Investment Agent smoke test, mark it live and use recorded OpenD by default. | Task 2, Task 3 | Not needed; no V1.2 live smoke was added |
| H4 | Document any new live command in `CONNECTOR_TESTS.md`. | H3 | Not needed |

### EC3: Docs describe V1.1 complete and V1.2 skeleton complete

| Task | Description | Depends on | Test |
| --- | --- | --- | --- |
| G | Update `ACTION_PLAN.md` after each V1.2 task status change. | All tasks | Done |
...
| G5 | Update `TESTING.md` with new V1.2 test files and retirement plan for legacy prototype tests. | A through E | Done |
| G6 | Add a V1.2 closeout section or new closeout doc only when implementation is complete. | H | Done |
...
- Terminal V1.2 Investment Agent review with fake/stub sentiment.
- Chat V1.2 Investment Agent review with fake/stub sentiment.
...
- Hosted LLM or live OpenD dependency leakage will make the deterministic suite
  unreliable.
- V1.2 docs should describe what was actually implemented, not the future Neo4j
  system.
