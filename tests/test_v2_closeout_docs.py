...
def test_v2_readme_records_closeout_and_remaining_stubs():
    ...
    assert "What Changed From V1" in text
...
def test_action_plan_and_decision_log_mark_v2_skeleton_complete():
    action_plan = (DOCS / "ACTION_PLAN.md").read_text(encoding="utf-8")
    decision_log = (DOCS / "DECISION_LOG.md").read_text(encoding="utf-8")

    assert "V2 skeleton is complete as of 2026-06-15" in action_plan
    assert "V2 skeleton is complete" in decision_log
    assert "V2 | Complete skeleton" in decision_log
